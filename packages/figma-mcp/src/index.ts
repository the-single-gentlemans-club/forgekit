...
export async function syncTheme(args: {
  ...
  if (!token) {
    throw new Error(
      'Missing figmaAccessToken. Provide it via arguments, env var FIGMA_ACCESS_TOKEN, or forgekit.json'
    )
  }
  ...
  if (!config?.outputs || config.outputs.length === 0) {
    ...
    const filesWritten = await writeThemeFile(
      theme,
      args.outputDir || config?.outputDir || './src/theme',
      args.framework || config?.framework || 'chakra'
    )
...
export async function runServer() {
  ...
  server.tool(
    'sync-theme',
    {
      ...
      framework: z
        .enum(['chakra', 'chakra-v3'])
        .optional()
        .describe('The target Chakra UI version'),
    },
    async (args) => {
      try {
