...
export async function writeThemeFile(
  theme: ThemeToken,
  outputDir: string,
  framework: 'chakra' | 'chakra-v3' = 'chakra',
  config?: ChakraMCPConfig
): Promise<string[]> {
  ...
  if (!config?.outputs || config.outputs.length === 0) {
    ...
    const files = await chakraWriter.write(theme, outputDir, {
      version: framework === 'chakra-v3' ? 'v3' : 'v2',
    })
  ...
  for (const output of config.outputs) {
    if (output.format === 'chakra' || output.format === 'chakra-v3') {
      ...
      const files = await writer.write(theme, output.dir, {
        version: output.format === 'chakra-v3' ? 'v3' : 'v2',
      })
      filesWritten.push(...files)
